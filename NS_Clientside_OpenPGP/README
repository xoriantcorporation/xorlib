...
We have overriden the function 'readPrivateKey' to get binary conversion of Private armoredKey in Uint8Array.
  ...
  return input;
  'input' will hold the uint8array of the private key.

We have overriden the function 'readKey' to get the binary conversion of public armoredKey in Uint8Array
  let input;
  if (armoredKey) {
    const { type, data } = await unarmor(armoredKey, config);
 ...
 return input;
 'input' will hold the uint8array of the public key
