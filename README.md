...
```
#### Modification in OpenPGP to support NetSuite Framework(Rhino Engine)

#1 We have created custom function for base64 encode & decode because standard JavaScript atob & btoa functions are not supported in NetSuite framework
-Function Calling:-
```js
encodeChunk = buf => base64encode(util.uint8ArrayToString(buf));
decodeChunk = str => util.stringToUint8Array(base64decode(str));
```	
#2 We have modified a encodeUTF8 function  which convert a native JavaScript string to a Uint8Array of utf8 bytes
```js
encodeUTF8
var buf = new ArrayBuffer(str.length*1); 
	...
	return bufView;
```	
#3 We have modified a decodeUTF8 function which convert a Uint8Array of utf8 bytes to a native JavaScript string
```js
decodeUTF8
var result = "";
  ...
  return result;
```
#4 We have modified  getRandomBytes function to retrieve secure random byte array of the specified length
function getRandomBytes- 
```js
var arr = '1234567890abcdefghijklnopqrstuvwxyz';
var bufArr = new ArrayBuffer(length*1); 
...
return bufView;
```
returning bufView Uint8Array as a output for the function.
```
